...
class IngestionService:
    ...
    async def process_text(self, text: str, metadata: Optional[Dict[str, Any]], user_id: str) -> int:
        ...
        Args:
            text: 原始文本
            metadata: 可选元数据（如 source/course/url 等）
            user_id: 用户隔离 ID
        """
        ...
        return await self.rag_service.ingest_text(raw_text=cleaned, source_name=source_name, user_id=user_id)

    async def process_file(self, file_content: bytes, filename: str, user_id: str) -> int:
        """
        摄入 PDF 文件到向量库（由底层 VectorStore 负责 PDF 解析/切分/向量化）。
        """
        if not filename or not filename.lower().endswith(".pdf"):
            raise ValueError("仅支持 PDF 文件格式")
        return await self.rag_service.ingest_pdf(file_content=file_content, filename=filename, user_id=user_id)
