...
async def generate_outline_task(
    ...
    try:
        ...
        if not user_id:
            print(
                "⚠️ [WARNING] generate_outline_task - user_id not provided, RAG retrieval will be skipped"
            )
...
async def generate_cheat_sheet_task(ctx, **kwargs) -> Dict[str, Any]:
    """
    ARQ 任务：生成小抄（全流程）
    ...
    try:
        ...
        if not user_id:
            raise ValueError(
                "user_id 是必需的，但未在任务参数中提供。请确保 API 请求包含 X-User-ID header。"
            )
        ...
        service = CheatSheetService.default()
        result_data = await service.create_cheat_sheet_flow(
            generate_request, user_id=user_id, metadata=metadata
        )
...
async def generate_pdf_task(ctx, cheat_sheet: Dict[str, Any]) -> Dict[str, Any]:
    """
